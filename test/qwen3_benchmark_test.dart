...
double _wordErrorRate(String hypothesis, String reference) {
  ...
  for (int i = 0; i <= m; i++) { dp[i][0] = i.toDouble(); }
  for (int j = 0; j <= n; j++) { dp[0][j] = j.toDouble(); }
  for (int i = 1; i <= m; i++) {
    for (int j = 1; j <= n; j++) {
...
void main() {
  final shouldRun = Platform.environment['QWEN3_BENCHMARK'] == 'true';

  if (!shouldRun) {
    test('skip (set QWEN3_BENCHMARK=true to run)', () {});
    return;
  }

  const modelId = 'qwen3-asr-0.6b-int8';
  final home = Platform.environment['HOME'] ?? '/tmp';
  final modelDir = Platform.environment['QWEN3_MODEL_DIR'] ??
  ...
  setUpAll(() async {
    if (!File(enWav).existsSync()) {
      throw Exception('WAV file not found: $enWav\nRun from the package root directory.');
    }
    ...
    if (!already) {
      print('Downloading $modelId (~935 MB) to: $modelDir');
    ...
    try {
      ...
      nativeAvailable = false;
      print('SKIPPING: model initialization failed — $e');
    }
  });
  ...
  group('Qwen3-ASR benchmark', () {
    test('hello_en.wav — English transcription & WER', () async {
      ...
      final hypShort = result.text.length > 300
          ...
          : result.text;
      final refShort = expected.length > 300
          ? '${expected.substring(0, 300)}…'
          : expected;
      print('--- Hypothesis ---');
      print(hypShort);
      print('');
    ...
    test('podcast_fr.wav — French transcription & WER', () async {
      ...
      print('');
      print('=== Qwen3-ASR Benchmark: podcast_fr.wav (French, 16kHz) ===');
      print('Inference time : ${result.inferenceTimeMs.toStringAsFixed(0)} ms');
      print('Confidence     : ${result.confidence?.toStringAsFixed(4) ?? "?"}');
      print('Duration       : ${result.durationMs?.toStringAsFixed(0) ?? "?"} ms');
      print('Language       : ${result.lang ?? "?"}');
      ...
      expect(result.text.isNotEmpty, isTrue);
      expect(wer, lessThan(1.0));
    }, timeout: const Timeout(Duration(minutes: 70)));
  });
}
